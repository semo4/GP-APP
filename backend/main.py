...
def evaluate_faithfulness(article, context):
    test_case = LLMTestCase(
        input=context, actual_output=article, retrieval_context=[context]
    )
...
def evaluate_style_matching(generated_article: str, past_articles_for_evaluation: list):
    ...
    try:
        ...
        embeddings = style_model.encode(
            [generated_article, combined_past], convert_to_tensor=True
        )
...
async def ingest_data(items: list[dict]):
    ...
    try:
        ...
        for item in items:
            ...
            vectors.append(
                {"id": item_id, "values": vector_values, "metadata": metadata}
            )
...
def remove_existing_signature(article_text, signature_keyword="By"):
    ...
    cleaned_text = re.sub(
        r"\n\s*" + re.escape(signature_keyword) + r".*$",
        "",
        article_text,
        flags=re.IGNORECASE,
    )
...
"""
    ...
    try:
        ...
        faithfulness_score = evaluate_faithfulness(
            generated_article_with_signature, retrieval_context
        )
        contextual_relevancy_score = evaluate_contextual_relevancy(
            generated_article_with_signature, retrieval_context
        )
        past_articles_for_evaluation = previous_articles + exported_articles
        style_matching_results = evaluate_style_matching(
            generated_article_with_signature, past_articles_for_evaluation
        )
        ...
        try:
            ...
            print(
                "✅ Successfully saved generated article to Firestore (previousArticles)."
            )
        ...
        print(
            f"contextual_relevancy_score: {evaluation_metrics['contextual_relevancy_score']}"
        )
        print(
            f"style_similarity_score: {evaluation_metrics['style_matching']['style_similarity_score']}"
        )
...
if not firebase_admin._apps:  # Prevent initializing multiple times
    cred = credentials.Certificate(
        "./credentials/gennews-2e5b4-firebase-adminsdk-k3adz-af7308d3ec2.json"
    )
...
async def get_user_articles(user_id: str):
    ...
    try:
        ...
        if doc.exists:
            data = doc.to_dict()
            return {
                "previousArticles": data.get(
                    "previousArticles", []
                ),  # Ensure default empty list
                "exportedArticles": data.get(
                    "exportedArticles", []
                ),  # Ensure default empty list
            }
        else:
...
NLTK_DATA_PATH = os.path.join(
    os.getcwd(), "venv", "lib", "python3.12", "site-packages", "nltk_data"
)
...
def extract_linguistic_print(previous_articles, exported_articles):
    ...
    passive_voice_count = sum(
        1 for word in words if word in {"by", "was", "were", "had been"}
    )
    active_voice_count = sum(
        1 for word in words if word in {"do", "does", "did", "has", "have", "had"}
    )
        ...
        if isinstance(google_data, dict):
            for item in safe_get(google_data, "news_results", [])[:2]:
                if isinstance(item, dict):
                    sources.append(
                        {"source": "Google News", "url": item.get("link", "#")}
                    )
    ...
    try:
        if topic:
            ...
            for topic in selected_topics:
                ...
                articles.append(
                    {
                        "title": newsapi_articles[0]["title"]
                        if newsapi_articles
                        else topic,
                        "content": summary,
                        "sources": sources,
            ...
            raise HTTPException(
                status_code=422, detail="Missing required query: topic or user_id"
            )
...
async def delete_articles(
    user_id: str = Query(..., description="User ID of the journalist"),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error deleting articles: {str(e)}"
        )
    # the new API ////////////////////////////////////////////////
            ...
            [
                {
                    ...
                    "author": user_dict.get(tweet["author_id"], {}).get(
                        "username", "Unknown"
                    ),
                    "verified": user_dict.get(tweet["author_id"], {}).get(
                        "verified", False
                    ),
                }
                for tweet in tweets
                if tweet["public_metrics"]["like_count"] >= 1000
                and user_dict.get(tweet["author_id"], {}).get("verified", False)
            ],
...
def generate_unsplash_prompt(title: str, content: str) -> str:
    ...
    return (
        title + " " + content[:50]
    )  # Example: combine title with first 50 characters of content
...
def get_twitter_images(topic: str):
    ...
    if response.status_code == 200:
        ...
        media_dict = {
            item["media_key"]: item["url"] for item in media if item["type"] == "photo"
        }
...
def fetch_newsapi_images(query: str, min_required: int = 3, max_results: int = 9):
    try:
        ...
        if len(image_urls) < min_required:
            print(
                f"⚠️ Only found {len(image_urls)} image(s), less than required minimum."
            )
        else:
